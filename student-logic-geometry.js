...
//GEOMETRY TEST CODE BELOW

function generateEquation() {
  const a = Math.floor(Math.random() * 13) + 3; // Random length for side a (3-15)
  const b = Math.floor(Math.random() * 13) + 3; // Random length for side b (3-15)
  document.getElementById("equation").textContent = `a = ${a}, b = ${b}`;
  return { a, b }; //generate a random equation for a geometry question
}

function solveForX(a, b) {
  return Math.sqrt(a ** 2 + b ** 2); // return answer/ return hypotenuse
}

  // Function to check the user's solution
function checkSolution() {
  const userInput = parseFloat(document.getElementById("hypotenuseInput").value); // Get user input and convert to a float
  const correctSolution = solveForX(equation.a, equation.b); // Calculate the correct solution
  ...
  if (isNaN(userInput)) { // Check if the input is not a valid number
      ...
      solutionElement.textContent = `Correct! The hypotenuse is ${correctSolution.toFixed(2)}.`;
      score++; // Increment the score if correct
    } else { // If the input is incorrect
      solutionElement.textContent = `Incorrect. The correct hypotenuse is ${correctSolution.toFixed(2)}.`;
    }
  ...
  function generateNewQuestion() {
      document.getElementById("hypotenuseInput").value = "";
