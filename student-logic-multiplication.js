...
//MULTIPLICATION TEST CODE BELOW

function generateEquation() {
  const a = Math.floor(Math.random() * 90) + 10; // Random number between 10-99
  const b = Math.floor(Math.random() * 90) + 10; // Random number between 10-99
  document.getElementById("equation").textContent = `${a} × ${b}`;
  return { a, b }; //generate a random equation for a multiplication question
}

function solveForX(a, b) {
  return a * b; // return answer/ return product
}

  // Function to check the user's solution
function checkSolution() {
  const userInput = parseInt(document.getElementById("answerInput").value); // Get user input and convert to an integer
  ...
  if (isNaN(userInput)) { // Check if the input is not a valid number
    ...
    } else if (userInput === correctSolution) { // Check if the input matches the correct solution
      solutionElement.textContent = `Correct! The answer is ${correctSolution}.`;
      score++; // Increment the score if correct
    } else { // If the input is incorrect
      solutionElement.textContent = `Incorrect. The correct answer is ${correctSolution}.`;
    }
